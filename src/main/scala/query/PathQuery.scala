      ...
      def nextVertexList(topologyIter:Iterator[(Int,Array[Int])],vertexIter:Iterator[(Int,Queue[Int])]):Iterator[(Int,Queue[Int])] ={
        ...
        def getStartAndCount(vertex:Int, edgeType:Byte, direction:String):(Int,Int)={
         ...
         vertexIter.map(vertex=>{
          ...
          getNeighbors(vertex._1,start,count).filter(filterVertex(_,nextQuery)).flatMap(x=>Iterator((x,vertex._2.enqueue(vertex._1))))
          //val item = topologyIter.toMap.getOrElse(vertex._1,Array())(0)
          //(item,query(queryIndex)._3::vertex._2)
        }).flatten
